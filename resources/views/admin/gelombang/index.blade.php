                        ...
                        <table id="datatables" class="table table-bordered table-striped">
                            ...
                            @foreach($gelombang as $data)
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn {{ $data->status == 0 ? "btn-success" : "btn-warning" }} btn-xs" href="{{ route('admin.gelombang.aktif', $data->id_gelombang) }}" @if($data->status == 0) onclick="return confirm('Yakin ingin MENGAKTIFKAN Gelombang ini?')" data-toggle="tooltip" data-placement="top" title="Aktifkan Gelombang Ini" @else onclick="return confirm('Yakin ingin MENONAKTIFKAN Gelombang ini?')" data-toggle="tooltip" data-placement="top" title="Nonaktifkan Gelombang Ini"@endif>@if($data->status == 0)<i class="nav-icon fas fa-check"></i> Aktifkan @else <i class="nav-icon fas fa-times"></i> Nonaktifkan @endif</a>
